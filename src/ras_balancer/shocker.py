...
class MatrixShocker:
    ...
    def shock_row_totals(
        ...
        for row_idx, magnitude in zip(row_indices, magnitudes):
            row_values = shocked[row_idx].toarray().ravel() if sp.issparse(shocked) else shocked[row_idx]
    ...
    def shock_column_totals(
        ...
        shocked = result.shocked_matrix.transpose() if sp.issparse(result.shocked_matrix) else result.shocked_matrix.T
    ...
    def shock_proportional(
        ...
        return self._create_shock_result(
            matrix, shocked, ShockType.PROPORTIONAL, magnitude,
    ...
    def shock_random(
        ...
        return self._create_shock_result(
            matrix, shocked, ShockType.RANDOM, magnitude,
            np.column_stack((shock_rows, shock_cols))
        )
